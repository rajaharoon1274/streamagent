...
const FROM = process.env.RESEND_FROM_EMAIL || 'notifications@streamagent.io'

/* ─────────────────────────────────────────────────────────────────────────────
   SHARED WRAPPER  — all emails share this branded shell
───────────────────────────────────────────────────────────────────────────── */
    ...
    <tr>
      <td align="center" style="padding:40px 20px;">
        <table width="560" cellpadding="0" cellspacing="0"
          ...
          <!-- Header -->
          <tr>
            <td style="padding:28px 40px 22px;border-bottom:1px solid rgba(255,255,255,0.06);">
...
export async function sendVerificationEmail({ email, firstName, code }) {
  const html = emailWrapper(`
    <h1 style="margin:0 0 8px;font-size:24px;font-weight:800;color:#EEF2FF;">Verify your email</h1>
    ...
    <div style="background:#1E2540;border:1px solid rgba(79,110,247,0.3);
      ...
      <div style="font-size:44px;font-weight:800;letter-spacing:12px;
        color:#EEF2FF;font-family:'Courier New',monospace;">${code}</div>
    </div>
    <p style="margin:0 0 8px;font-size:13px;color:#7B87A0;">
...
export async function sendPasswordResetEmail({ email, firstName, resetUrl }) {
  const html = emailWrapper(`
    <h1 style="margin:0 0 8px;font-size:24px;font-weight:800;color:#EEF2FF;">Reset your password</h1>
    <p style="margin:0 0 28px;font-size:14px;color:#7B87A0;line-height:1.6;">
      Hi ${firstName || 'there'},<br/>
    ...
    <div style="background:#1E2540;border:1px solid rgba(255,255,255,0.06);
      ...
      <p style="margin:0 0 6px;font-size:11px;font-weight:700;color:#7B87A0;text-transform:uppercase;letter-spacing:1px;">
        Or copy this link
      </p>
      <p style="margin:0;font-size:12px;color:#4F6EF7;word-break:break-all;">${resetUrl}</p>
    </div>
    <p style="margin:0 0 8px;font-size:13px;color:#7B87A0;">
...
/* ─────────────────────────────────────────────────────────────────────────────
   3. LEAD MAGNET / CTA-DOWNLOAD EMAIL  ← NEW IN DAY 12
   Triggered when a cta-download element has requireEmail=true + downloadUrl set.
───────────────────────────────────────────────────────────────────────────── */
export async function sendLeadMagnetEmail({
  ...
  const html = emailWrapper(`
    ...
    <p style="margin:0 0 24px;font-size:14px;color:#7B87A0;line-height:1.6;">
      Hi ${displayName},<br/>
      Thanks for watching${videoTitle ? ` <strong style="color:#EEF2FF;">${videoTitle}</strong>` : ''}!
      Your requested resource is ready to download.
    </p>
    ...
    <!-- Fallback link -->
    <div style="background:#1E2540;border:1px solid rgba(79,110,247,0.2);
      border-radius:10px;padding:14px 18px;margin-bottom:24px;">
      <p style="margin:0 0 5px;font-size:10px;font-weight:700;color:#7B87A0;
        text-transform:uppercase;letter-spacing:1px;">Direct link</p>
      <a href="${downloadUrl}"
        style="font-size:12px;color:#4F6EF7;word-break:break-all;
          text-decoration:none;">${downloadUrl}</a>
    </div>

    <p style="margin:0;font-size:12px;color:#3D4560;line-height:1.6;">
      This link was generated by <strong style="color:#7B87A0;">${agentName}</strong> via StreamAgent.
      If you did not request this download, you can safely ignore this email.
    </p>
  `)

  const { data, error } = await resend.emails.send({
    from: `${agentName} via StreamAgent <${FROM}>`,
    to: [email],
    subject: `Your download: ${fileName}`,
    html,
  })

  if (error) {
    console.error('[Email] Lead magnet send failed:', error)
    throw new Error(`Lead magnet email failed: ${error.message}`)
  }

  console.log(`[Email] Lead magnet sent to ${email}: ${fileName}`)
  return data
}