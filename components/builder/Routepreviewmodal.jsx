...
// ─── Component ────────────────────────────────────────────────────────────────
// Props:
//   nodes       — full node array for this route (from DB via useRoutes)
//   routeEls    — { nodeId: element[] }
//   videos      — videos array (from useWorkspaceVideos)
//   mode        — 'desktop' | 'mobile'
//   onClose     — () => void
export default function RoutePreviewModal({ nodes = [], routeEls = {}, videos = [], mode = 'desktop', onClose }) {
  ...
  // Support both DB shape (duration_seconds) and palette shape (dur string)
  const video = node?.videoId ? videos.find(v => v.id === node.videoId) : null
  const dur = video
    ? (video.duration_seconds || parseDur(video.dur))
    : (node?.duration || 120)
  ...
  const tick = useCallback(() => {
    ...
    const d = vid ? (vid.duration_seconds || parseDur(vid.dur)) : (n?.duration || 120)
    const newP = +(s.progress + 0.25).toFixed(2)

    // Check choice point triggers
    let shouldShowChoices = false
    const edgesNow = n ? getEdgesForNode(n) : []
    if (!s.showingChoices && edgesNow.length > 0) {
      if (n?.choicePoints?.some(cp => newP >= (cp.triggerAt || d * 0.9))) shouldShowChoices = true
      if ((newP / d) * 100 >= 90) shouldShowChoices = true
    }

    // End of video
    if (newP >= d && !shouldShowChoices) {
    ...
    if (shouldShowChoices) {
      clearInterval(intervalRef.current)
      setSt(prev => ({ ...prev, progress: newP, playing: false, showingChoices: true }))
      return
    }

    setSt(prev => ({ ...prev, progress: newP }))
  }, [nodes, videos])
  ...
  function chooseNext(targetId, label, color) {
    ...
    const targetNode = nodes.find(r => r.id === targetId)

    // If the target has a real video, open public player in new tab
    if (targetNode?.videoId) {
      window.open(`/watch/${targetNode.videoId}?route_node=${targetId}`, '_blank')
      return
    }

    // Otherwise continue in-modal simulation
    setSt(prev => ({
      nodeId: targetId,
      progress: 0,
      playing: true,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', background: '#080c18' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', padding: mode === 'mobile' ? '8px' : '24px', position: 'relative' }}>
          <div style={{ position: 'relative', width: '100%', maxWidth: maxW, aspectRatio: arCss, background: `linear-gradient(135deg,${node?.color || '#4F6EF7'}15,${node?.color || '#4F6EF7'}08)`, borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(255,255,255,.06)', boxShadow: '0 20px 60px rgba(0,0,0,.5)' }}>
            ...
            {/* Video placeholder / open real player CTA */}
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
              ...
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,.4)', marginBottom: node?.videoId ? 14 : 0 }}>
                {video?.dur || fmt(dur)} · {node?.isRoot ? 'Entry Point' : 'Route Video'}
              </div>
              {/* If node has a real video, show "Open Full Player" */}
              {node?.videoId && (
                <div style={{ pointerEvents: 'all' }}>
                  <button
                    onClick={() => window.open(`/watch/${node.videoId}?route_node=${node.id}`, '_blank')}
                    style={{ padding: '8px 20px', borderRadius: 8, background: node?.color || '#4F6EF7', border: 'none', color: '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: `0 4px 16px ${node?.color || '#4F6EF7'}55` }}>
                    ▶ Open Full Player
                  </button>
                </div>
              )}
            ...
            {/* Overlay elements (non-fullscreen) */}
            {visEls.map(el => {
            ...
            {visEls
              .filter(el => ['funnel-urgency', 'cta-email', 'cta-booking', 'cta-download', 'survey-poll', 'survey-rating', 'survey-nps'].includes(el.type))
              .map(el => {
                const p = el.props || {}
                  ...
                  <div key={el.id} style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg,rgba(14,20,40,.95),rgba(8,15,32,.95))', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 5, animation: 'saFadeIn .3s ease' }}>
                    <div style={{ background: 'rgba(7,9,15,.92)', border: '1px solid rgba(255,255,255,.1)', borderRadius: 16, padding: '20px 24px', width: '88%', maxWidth: 300, textAlign: 'center', boxShadow: '0 24px 64px rgba(0,0,0,.6)' }}>
                      <div style={{ fontSize: 24, marginBottom: 10 }}>
                        {el.type === 'cta-booking' ? '📅' : el.type === 'cta-download' ? '⬇️' : el.type.startsWith('survey') ? '📊' : '✉️'}
                      </div>
