...
export default function BuilderElementsEditor({
  ...
  useEffect(() => {
    if (node?.id !== prevNodeId.current) {
      ...
      setCurTime(0)
      setPlaying(false)
    } else if (initEls && initEls.length > 0 && elements.length === 0) {
      setElements(initEls)
    }
  }, [node?.id, initEls])
  ...
  // ── Element CRUD ─────────────────────────────────────────────────────────────
  const addEl = useCallback(async type => {
    ...
    if (onCreateElement) {
      try {
        const savedEl = await onCreateElement(el)   // returns element with real DB UUID
        // Replace temp bel- id with real DB id everywhere
        setElements(prev => {
          const next = prev.map(e => e.id === el.id ? { ...e, id: savedEl.id } : e)
          onChange(next)
          return next
        })
        setSelectedId(savedEl.id)
      } catch (err) {
        console.error('[BuilderElementsEditor] Create element failed:', err)
        // Remove the optimistic element on failure
        setElements(prev => {
          const next = prev.filter(e => e.id !== el.id)
          onChange(next)
          return next
        })
      }
    }
  }, [onChange, onCreateElement])
  ...
  function handleChange(updated) {
    ...
    if (onUpdateElement && updated.id && !updated.id.startsWith('bel-')) {
      onUpdateElement(updated.id, updated)
    }
  }
  ...
  function handleDelete(id) {
    ...
    if (onDeleteElement && id && !id.startsWith('bel-')) {
      onDeleteElement(id)
    }
  }
