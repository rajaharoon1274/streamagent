...
//   onNodeChange — (nodeId, patch) => void  — updates node via Builder (hits API)
export default function BuilderConnections({ node, allNodes, edges, onNodeChange }) {
  ...
  async function connectTo(targetId) {
    ...
    setSaving(true)
    try {
      await onNodeChange(node.id, {
        choicePoints: [...(node.choicePoints || []), newCP],
      })
      toast.success(`Connected → ${target.title}`)
    } catch (err) {
  ...
  async function disconnectFrom(targetId) {
    ...
    setSaving(true)
    try {
      await onNodeChange(node.id, { choicePoints: updated })
  ...
  async function addBlankCP() {
    ...
    try {
      ...
      toast.success('Choice point added')
    } catch (err) {
  ...
  return (
    <div style={{ padding: 14 }}>
      ...
      <button onClick={addBlankCP} disabled={saving}
        style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px 12px', borderRadius: 9, background: 'var(--acc)', color: '#fff', fontSize: 11, fontWeight: 700, border: 'none', cursor: saving ? 'wait' : 'pointer', marginBottom: 14, fontFamily: 'inherit', opacity: saving ? 0.7 : 1 }}>
        {saving ? '…' : '⑂'} Add Choice Point
      </button>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          {myEdges.map(e => (
            <div key={`${e.from.id}-${e.choice.id}`}
              ...
              <button onClick={() => disconnectFrom(e.to.id)} disabled={saving}
      ...
      {otherNodes.length > 0 ? (
        <div>
          ...
          {otherNodes.map(n => {
            ...
            return (
              <div key={n.id}
                onClick={() => !saving && connectTo(n.id)}
                style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 9, background: isConnected ? `${n.color}14` : 'var(--s2)', border: `1.5px solid ${isConnected ? n.color : 'var(--b2)'}`, marginBottom: 6, cursor: saving ? 'wait' : 'pointer', transition: 'all .15s' }}
                onMouseOver={e => { if (!isConnected && !saving) { e.currentTarget.style.borderColor = n.color + '88'; e.currentTarget.style.background = `${n.color}08` } }}
