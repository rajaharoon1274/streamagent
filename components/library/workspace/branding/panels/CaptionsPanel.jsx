...
const CAPTION_STYLES = [
  { id: 'hormozi', label: 'Hormozi',  desc: 'Bold word-by-word' },
  { id: 'classic', label: 'Classic',  desc: 'Bottom bar' },
  { id: 'modern',  label: 'Modern',   desc: 'Clean sans-serif' },
  { id: 'minimal', label: 'Minimal',  desc: 'Subtle, small' },
]
...
export default function CaptionsPanel() {
  ...
  const style      = state.aiCaptionStyle || 'hormozi'
  ...
  const updateCaption = (i, text) => {
    const next = captions.map((c, ci) => ci === i ? { ...c, text } : c)
    set({ aiCaptions: next })
  }

  const removeCaption = (i) => set({ aiCaptions: captions.filter((_, ci) => ci !== i) })

  const addCaption = () => set({
    aiCaptions: [...captions, { start: 0, end: 5, text: 'New caption' }]
  })
    ...
    <>
      {captions.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '12px 0' }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--t1)', marginBottom: 4 }}>No Captions Yet</div>
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 10 }}>
            <button onClick={generate} style={{ flex: 1, padding: '6px 4px', borderRadius: 8, background: 'var(--s3)', border: '1px solid var(--b2)', color: 'var(--t2)', fontSize: 10, fontWeight: 600, cursor: 'pointer' }}>
              ↻ Regenerate
            </button>
            <button
              onClick={() => {
                const content = captions.map((c, i) => `${i + 1}\n${fmt(c.start)} --> ${fmt(c.end)}\n${c.text}`).join('\n\n')
      ...
      {/* ── Style ── */}
      <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 6 }}>Caption Style</div>
      {CAPTION_STYLES.map(st => {
        const act = style === st.id
        return (
          <div key={st.id} onClick={() => set({ aiCaptionStyle: st.id })}
            style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 8, marginBottom: 4, cursor: 'pointer', border: `1px solid ${act ? 'var(--acc)' : 'var(--b2)'}`, background: act ? 'rgba(79,110,247,0.06)' : 'var(--s3)' }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', border: `2px solid ${act ? 'var(--acc)' : 'var(--b2)'}`, background: act ? 'var(--acc)' : 'transparent', flexShrink: 0 }} />
            <div>
              <div style={{ fontSize: 11, fontWeight: act ? 700 : 500, color: act ? 'var(--acc)' : 'var(--t1)' }}>{st.label}</div>
      ...
      {/* ── Colors ── */}
      <div style={{ height: 10 }} />
      <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--t3)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 6 }}>Colors</div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--t2)' }}>Highlight</span>
        <input type="color" value={hlColor} onChange={e => set({ aiHighlightColor: e.target.value })}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        <input type="color" value={bgColor} onChange={e => set({ aiBarColor: e.target.value })}
          style={{ width: 26, height: 26, borderRadius: 5, border: '1px solid var(--b2)', padding: 2, background: 'var(--s3)', cursor: 'pointer' }} />
      </div>
      ...
      <div style={{ display: 'flex', gap: 4 }}>
        {OPACITY_PRESETS.map(p => {
          ...
          return (
            <button key={p.v} onClick={() => set({ aiCaptionOpacity: p.v })}
              style={{ flex: 1, padding: 4, borderRadius: 5, fontSize: 9, fontWeight: 600, border: `1px solid ${act ? 'var(--acc)' : 'var(--b2)'}`, background: act ? 'rgba(79,110,247,0.1)' : 'var(--s3)', color: act ? 'var(--acc)' : 'var(--t2)', cursor: 'pointer' }}>
              {p.l}
            </button>
          )
        })}
